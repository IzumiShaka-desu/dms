...
<script>
	...
	function refreshDataTable() {
		$("#DataTables_Table_1").DataTable().clear().draw();
		...
		dataTableRows.forEach(function callback(row, index) {
			...
			$("#DataTables_Table_1").DataTable().row.add(cells).draw();
		});
	...
	function refreshDataTableAdd() {
		...
		$("#DataTables_Table_1").DataTable().row.add(formsCell).draw();
	...
	$(document).ready(function() {
		...
		if (isAddDocuments == true || isAddDocumentsLisensi == true) {
			refreshDataTableAdd();
			refreshDataTableAddLisensi();
		} else {
			// refreshDataTable();
